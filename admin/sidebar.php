<?php
// admin/sidebar.php — Enterprise Responsive Sidebar
// Ek hi file mein poora mobile+desktop responsive system
...
<?php endif; ?>
  ...
  <nav class="adm-nav">
    ...
    <!-- Dashboard -->
    <div class="adm-nav-lbl">Dashboard</div>
    ...
    <a href="<?= BASE_URL ?>/admin/index.php?tab=plans" class="adm-link <?= adm_active([], ['plans']) ?>">
    ...
    <a href="<?= BASE_URL ?>/admin/index.php?tab=catalog" class="adm-link <?= adm_active([], ['catalog']) ?>">
    ...
    <a href="<?= BASE_URL ?>/admin/smtp.php" class="adm-link <?= adm_active(['smtp.php']) ?>">
      ...
      SMTP Email
      <?php try { $__sp=(int)db()->query("SELECT COUNT(*) FROM smtp_orders WHERE status='pending'")->fetchColumn(); if($__sp>0) echo '<span class="adm-n-badge">'.$__sp.'</span>'; } catch(\Exception $e) {} ?>
    </a>
    <a href="<?= BASE_URL ?>/admin/proxy.php" class="adm-link <?= adm_active(['proxy.php']) ?>">
      ...
      Proxy Provider
      <?php try { $__sp=(int)db()->query("SELECT COUNT(*) FROM proxy_orders WHERE status='pending'")->fetchColumn(); if($__sp>0) echo '<span class="adm-n-badge">'.$__sp.'</span>'; } catch(\Exception $e) {} ?>
    </a>

    <!-- Users -->
    <div class="adm-nav-lbl">Users</div>
    ...
    <!-- Marketing -->
    <div class="adm-nav-lbl">Marketing</div>
    ...
    <a href="<?= BASE_URL ?>/admin/career.php" class="adm-link <?= adm_active(['career.php']) ?>">
      ...
      Careers
      <?php try { $__ca=(int)db()->query("SELECT COUNT(*) FROM career_applications WHERE status='pending'")->fetchColumn(); if($__ca>0) echo '<span class="adm-n-badge">'.$__ca.'</span>'; } catch(\Exception $e) {} ?>
    </a>

    <!-- Billing -->
    <div class="adm-nav-lbl">Billing</div>
    ...
    <a href="<?= BASE_URL ?>/admin/index.php?tab=kyc" class="adm-link <?= adm_active([], ['kyc']) ?>">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
      KYC Requests
      <?php try { $__kp=(int)db()->query("SELECT COUNT(*) FROM kyc_requests WHERE status='pending'")->fetchColumn(); if($__kp>0) echo '<span class="adm-n-badge">'.$__kp.'</span>'; } catch(\Exception $e) {} ?>
    </a>
    ...
    <a href="<?= BASE_URL ?>/admin/callback-requests.php" class="adm-link <?= adm_active(['callback-requests.php']) ?>">
      ...
      Callbacks
      <?php try { $__cb=(int)db()->query("SELECT COUNT(*) FROM callback_requests WHERE status='pending'")->fetchColumn(); if($__cb>0) echo '<span class="adm-n-badge">'.$__cb.'</span>'; } catch(\Exception $e) {} ?>
    </a>
    ...
    <!-- Analytics -->
    <div class="adm-nav-lbl">Analytics</div>
    ...
    <!-- Content / Legal -->
    <div class="adm-nav-lbl">Content</div>
    <a href="<?= BASE_URL ?>/admin/legal-pages.php" class="adm-link <?= adm_active(['legal-pages.php']) ?>">
      ...
      Legal Pages
      <?php try { $__lp=(int)db()->query("SELECT COUNT(*) FROM legal_pages WHERE is_published=0")->fetchColumn(); if($__lp>0) echo '<span class="adm-n-badge">'.$__lp.'</span>'; } catch(\Exception $e) {} ?>
    </a>
    ...
    <!-- Back to site -->
    <div style="margin-top:6px;padding-top:8px;border-top:1px solid rgba(255,255,255,.05)">
...
// Close sidebar when nav link is tapped on mobile
